...
- `product/create/` and `product/[productid]/edit/` — product create/edit forms (edit is admin/moderator-only).
- `r/[permalink]/page.tsx` — public reader view for a published `Ulasan` (blog post).
- `admin/*` — gated in `admin/layout.tsx` by `session.user.role` (`ADMIN` or `MODERATOR`); covers product moderation, category management, user management, reports, and Ulasan (blog) CRUD.
