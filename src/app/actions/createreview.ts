...
export async function createReview(formData: FormData) {
  try {
    ...
    const reviewer = (formData.get("created_by") as string) ?? null;
    ...
    if (!Number.isInteger(productId) || Number.isNaN(productId)) {
      return { success: false, error: "product_id tidak valid" };
    }
    if (!Number.isInteger(rating) || rating < 1 || rating > 5) {
      return { success: false, error: "rating harus integer antara 1 dan 5" };
    }
